...
This release turns the homepage's two static "Search Highlighting" and "Author Names" sections into live, interactive demonstrations that call the same production functions the `view0/1/2` filter pages use, then expands the SEO section with a direct answer to the 301-redirect question, then makes the author-normalization function actually correct on a real-world case (honorifics like *Dr.*) and rebuilds the author-normalization Part 1 around two real `USelect` dropdowns showing 20 messy raw CMS entries collapsing to 4 canonical authors. Renames demo authors to clearly fictional ones (Riley Calder, Morgan Whitcomb, Behavioral Outcomes & Research Group) so a manager can't mistake them for real ICJIA staff. Documentation on `/site` updated to match.

### Added

- **`HighlightSandbox.vue` — live search-highlighting sandbox on the homepage.** Slots into the emerald "Search Highlighting" section between the static "with highlighting" green card and the "Four properties" grid. Real text input pre-filled with `juvenile` so the widget shows highlighting on first paint; four preset chips (`juvenile`, `Cook County`, `diversion`, `recidivism`) pre-fill the input on click; `aria-pressed` shows which chip matches the current query. A ~150-word abstract about juvenile diversion in Cook County by *Riley Calder · Sept 2024* re-renders live with `<mark class="bg-primary/40">` wrappers around every match in title and abstract. Match-count badge (`aria-live="polite"`) updates to "no matches" / "1 match" / "N matches" / "type to highlight". Imports `highlightSegments()` directly from `~/utils/article-format` — the same function `ArticleCard.vue` uses in production. Pulsing `● Live · Try it yourself` header explicitly names the function and module path so a skim-reader can verify the integrity claim.
- **`AuthorNormalizationSandbox.vue` — live author-normalization sandbox on the homepage.** Slots into the violet "Author Names" section between the static "one canonical entry" violet card and the "Six normalization steps" grid. Two parts: **Part 1** is two real `USelect` dropdowns side-by-side, the same Nuxt UI widget `view1.vue` and `view2.vue` use in production. The left dropdown's closed-state placeholder reads *"All authors (20 entries) ▾"*, the right reads *"All authors (4 entries) ▾"* — the entire teaching point visible before either dropdown is opened. Both dropdowns are driven by a 20-row `SAMPLE_CMS_AUTHORS` array of messy raw entries with article counts (`Riley Calder (3)`, `Dr. Riley Calder (2)`, `··Riley··Calder·· (1)`, `Morgan Whitcomb, M.D. (2)`, `Behavioral Outcomes  &  Research Group (1)`, etc.); right side is computed live by passing every raw entry through `authorKey()` and grouping by canonical key, with merged counts shown in each option label. Below each dropdown: a footer summary, plus a live caption that updates when the user picks an entry — left shows the canonical key for the picked spelling, right shows the merged article count and number of raw spellings. **Part 2** is a 3-author tab selector (Riley Calder / Morgan Whitcomb / Behavioral Outcomes & Research Group) + variant chips + free-form input + 6-step pipeline (`<ol>` with `aria-live="polite"` on the canonical-key row) that runs `authorKeySteps()` live as the user types. Variant chips and pipeline rows use `visibleAbnormalWhitespace()` and `visibleWhitespace()` helpers so leading/trailing/double internal spaces are visible as `·` characters. Bottom assertion line: *"All N variants of '<canonical>' collapse to <key> ✓"* — recomputes per tab and serves as a runtime regression test on the sample data and the function.
- **`/` SEO section — "Why SEO matters. A lot." block.** New amber callout sits between the "Google rankings stay / AI citations stay / Backlinks stay" 3-card grid and the "summary === article" reasoning box. Three explicit teaching points: (1) the stakes — *"36,800 visits over twelve months. Most of them arrived via Google. Take SEO away and that traffic doesn't reroute somewhere else — it disappears."* (2) the compounding nature — *"SEO is earned, not given. Slowly. Hub 1.0's most-read articles average many years of accumulated trust. Reset the URL and the clock starts over at zero."* (3) **a direct answer to the 301-redirect FAQ** — *"What about 301 permanent redirects? Don't those preserve SEO?"* The block answers honestly: Google's official guidance says full ranking signal is preserved, but real-world SEO measurement consistently shows 5–15% loss in practice; 301s are damage-mitigation, not a get-out-of-jail-free card. Includes a plain-English ranking ladder so a manager sees the cost order at a glance: **stable URL → 301 → 302 → 404**. Closes with the catalog-as-entry-point framing: *"Lose SEO and you don't lose the homepage; you lose the hundreds of doors people use to find specific work. A research catalog without SEO is a library where the books exist but the aisles are sealed off."*
- **`authorKey()` Step 1 — "Drop honorifics".** New as the first transformation in the 6-step pipeline (was 5 steps). Strips leading honorific prefixes (`Dr., Prof., Professor, Mr., Mrs., Ms., Miss, Rev., Reverend, Hon., Honorable, Sir, Dame, Lord, Lady`) before the credentials cut. Regex `/^(Dr|Prof|Professor|Mr|Mrs|Ms|Miss|Rev|Reverend|Hon|Honorable|Sir|Dame|Lord|Lady)(\.\s*|\s+)/i` — case-insensitive, handles both `Dr Smith` and `Dr. Smith` and `Dr.Smith` (period without trailing space), rejects `Drone` / `Drilling` / `MS-13` (no period or whitespace boundary). Closes a real correctness gap: previously `Dr. Riley Calder` and `Riley Calder` produced different canonical keys (`dr. riley calder` vs `riley calder`), so the same person split across two dropdown entries. Now they merge.

### Changed

- **`/` "What the CMS delivers" static problem-list expanded from 7 entries to 8** — adds *Dr. Riley Calder* to the seven-spelling Riley Calder dump, italicized to flag the new honorific-stripping step. Section heading becomes *"Same person, listed eight different ways"*; the merged-result card now reads *"8 articles"* and *"all eight variants matched"*. The static section now matches the live sandbox below it (which uses 9 variants in the Part 2 sample data, including `Dr. Riley Calder` and `Dr. Riley Calder, Ph.D.`).
- **`/` "Five normalization steps" card grid → "Six normalization steps".** Adds a new violet card #1 *"Drop honorifics — Strip leading prefixes. Dr. Riley Calder → Riley Calder. Handles Dr., Prof., Mr., Mrs., Ms., Rev., Hon., etc."* and renumbers the existing five (Drop credentials → 2, Unify ampersands → 3, Collapse spaces → 4, Trim whitespace → 5, Lowercase → 6). Grid layout updated from `md:grid-cols-5` to `md:grid-cols-3 lg:grid-cols-6` for the extra card to wrap cleanly on tablet and stay one row on desktop.
- **`/` `authorKey()` pseudocode block** — adds the new Step 1 regex line at the top of the pseudocode `<pre>`, renumbers the existing five comments. Closing summary paragraph updated to describe the new 6-step structure: "Steps 1, 2, and 3 normalize *meaning* (honorifics and credentials are noise; '&' and 'and' are the same word). Steps 4 and 5 normalize *spacing*. Step 6 normalizes *case*. Order matters: dropping honorifics first lets the comma-cut work cleanly on whatever's left, regardless of how the name was prefixed."
- **`/` Solution 3 concern card** — *"Five small text-cleaning steps merge every spelling variant onto a single matched-up entry"* → *"Six small text-cleaning steps..."*.
- **Demo authors renamed throughout to clearly fictional names.** `Jane Carter` (8+ occurrences across `index.vue`, `HighlightSandbox.vue`, `AuthorNormalizationSandbox.vue`) → **Riley Calder** (matches the existing fake-name convention in `schema.vue` and `site.vue` pseudocode). `Marcus Reynolds` → **Morgan Whitcomb**. `Forensics & Policy Lab` → **Behavioral Outcomes & Research Group**. Plus a new fourth canonical *Center for Public Safety Research and Analysis* used in the 20-row CMS sample so the dropdown demo includes a long institutional name with the `&` ↔ `and` pattern. The rename is consistent: `JANE CARTER` → `RILEY CALDER`, `jane carter` (canonical) → `riley carter`, byline on the search-highlight sample → `By Riley Calder · Sept 2024`. Reason: a manager skimming the demo shouldn't be able to mistake the example names for real ICJIA staff.
- **`/site` `authorKey()` documentation updated to the 6-step pipeline.** Pseudocode `<pre>` block updated verbatim. The 5-card step grid becomes a 6-card grid with the new "Drop honorifics" card as #1 and the others renumbered. Section reveal phrase becomes *"Six chained string ops. One canonical key."* The before/after table grows from 8 rows to 9 — new top row `"Dr. Riley Calder" → "riley calder"` showing step 1 in action; existing rows renumbered (1→2, 3→4, 4→5, 5→6) in the *"step that did it"* column. All `Jane Carter` / `Jane Doe` references replaced with `Riley Calder` for consistency. The amber "When the function does NOT merge — by design" companion table renames its examples to `Riley Calder ≠ Robin Calder / Riley A. Calder / R. Calder / Riley Calder & Associates` (Robin chosen so the alliterative-confusion point — *"could be Riley, could be Robin"* — still lands).
- **`/` Part 1 author-normalization layout reframed from comparison cards to real `USelect` dropdowns.** The previous Part 1 used two `<details open>` elements styled to look like dropdowns — a manager would click them expecting select-style interaction, but the entries weren't clickable. Replaced with two real `USelect` widgets backed by computed item arrays. The closed-state placeholder shows the headline contrast (*"All authors (20 entries) ▾"* vs *"All authors (4 entries) ▾"*) so the punchline is visible before either dropdown is even opened. Picking an entry triggers a live caption below: left shows *"You picked: 'Dr. Riley Calder' → its canonical key is dr. riley calder"* (or `riley calder` after the new step 1 strips the honorific); right shows *"You picked: Riley Calder · 13 articles · merged from 9 raw spellings · canonical key: riley calder"*. Item labels visualize abnormal whitespace (`··Riley··Calder·· (1)` instead of the visually-identical `Riley Calder (1)` that HTML normally renders), and the sort uses trimmed names so leading whitespace doesn't push variants out of the alphabetical cluster. Same `authorKey()` from `~/utils/article-format` powers the right-side computation as powers the production filter on `view1` / `view2`.

### Why this matters

- The homepage's three solution sections now make the *"this is live, working code"* claim verifiable. A skim-reader can type into the search input or open either Authors dropdown and see the actual production functions running. Before this release, the three solution sections were rich static infographics; a careful manager could plausibly conclude the demo was rendered from screenshots. They no longer can.
- The honorific-stripping fix closes a real correctness gap. Before today, the same researcher entered as *Riley Calder* in some articles and *Dr. Riley Calder* in others would show up as two distinct authors in the production filter dropdown — the exact bug normalization is supposed to fix. Now every variant collapses to one entry.
- The fictional-author rename removes any chance that a reviewer reads *Jane Carter* as a real ICJIA staff member. The new names share the existing fake-name convention with `schema.vue` and `site.vue` so the whole demo reads as authored, not improvised.
